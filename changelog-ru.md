...
`*` Изменена логика запуска: теперь все расширения с отложенным запуском будут также запущены после включения Delayed Startup, а не только после запуска браузера (с настройкой <em>extensions.delayedStartup.startOnEnable</em> = true).<br>
